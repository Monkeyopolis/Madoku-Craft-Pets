...
public final class PetHudManagerClient {
	...
	private static void render(GuiGraphics context, DeltaTracker tickCounter) {
		...
		if (player == null || client.level == null || client.options.hideGui || player.isSpectator() || !PetConfigAPIManager.isEnabled()) return;
	...
	private static void renderScaledTexture(GuiGraphics context, Identifier texture, int x, int y) {
		renderScaledTexture(context, texture, x, y, ABILITY_CARD_SIZE);
	}

	private static void renderScaledTexture(GuiGraphics context, Identifier texture, int x, int y, int size) {
	...
	private static void renderCooldown(GuiGraphics context, Minecraft client, int slot, int cooldownIndex, long totalCooldownTicks, int cardX, int cardY) {
	...
	private static void renderCooldownNumber(GuiGraphics context, Minecraft client, int cardX, int cardY, long remainingTicks) {
		...
		context.drawString(client.font, cooldownText, Math.round(textX / textScale), Math.round(textY / textScale), ABILITY_COOLDOWN_TEXT_COLOR, true);
		context.pose().popMatrix();
	}
	...
	private static int[] computeAbilityColumnXs(GuiGraphics context, LocalPlayer player, int columnCount) {
